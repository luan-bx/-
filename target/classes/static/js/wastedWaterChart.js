var lineChartData = {
        labels : ["2022/02/22","2022/02/23","2022/02/24","2022/02/25","2022/02/26","2022/02/27","2022/02/28","2022/03/01","2022/03/02","2022/03/03","2022/03/04","2022/03/05"],
        datasets : [
            {
            ...
            },
             {
                ...
                pointColor : "rgba(151,187,205,1)",
                pointStrokeColor : "#fff",
                data : [28,48,40,19,96,27,100,13,49,35,48,67]
            }
        ]
...
function predict(){
    /*预测曲线和实际曲线首尾相接*/
    var startTime = new Date(document.getElementById('endTime').value);
    /*预测日期和数据*/
    let label = new Array();
    let data = new Array();
    let diff = dateDifference(new Date(document.getElementById('startTime').value),new Date(document.getElementById('endTime').value));
    document.getElementById("line").width=(diff+5)*40+100;
    for(var i=0;i<6;i++){
        if(i==0){
            data[i]=lineChartData.datasets[0].data[lineChartData.datasets[0].data.length-1];
        }
        else{
            label.push(startTime.toLocaleDateString().replace(startTime.getFullYear().toString()+"/",""));
            data.push(Math.floor(Math.random()*10+90));
    ...
    var predictLineCharData = {
        labels : label,/*默认预测五天，第一个点用来接实际曲线的尾巴*/
        datasets : [
            {
                fillColor : "rgba(151,187,205,0)",
                strokeColor : "rgba(151,187,205,1)",
    ...
    console.log(predictLineCharData);
    new Chart(document.getElementById("line").getContext("2d")).Line(predictLineCharData);
